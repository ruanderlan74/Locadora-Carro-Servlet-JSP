...
import br.com.ada.model.Entidade;
...
public class Repository<T extends Entidade>  {
    ...
    public T salvar(T entidade) {
        if (existe(entidade)) {
            throw new IllegalArgumentException("Já existe um Item com esse id cadastrado");
        }
        dados.add(entidade);
        return entidade;
    }
    ...
    public void atualizar(T entidade) {
        if (!existe(entidade)) {
            throw new IllegalArgumentException("Não existe um intem com esse ID cadastrado para atualizar");
        }
